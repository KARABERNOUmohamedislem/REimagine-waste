...
def Upload_Images(folder):
        ...
        for image in tagged_images['files']:
            ...
            for tag in image['tags']:
                ...
                regions.append(Region(
                    tag_id=tag_id, left=tag['left'], top=tag['top'], width=tag['width'], height=tag['height']))
                print("here1")
    ...
    upload_result = training_client.create_images_from_files(
        custom_vision_project.id, ImageFileCreateBatch(images=tagged_images_with_regions))
    # Check for failure
